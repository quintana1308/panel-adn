...
class Login extends Controllers{
			...
			if(empty($_POST['txtEmail']) || empty($_POST['txtPassword'])){
				...
				if(empty($requestUser)){
					...
					if($arrData['STATUS'] == 1){
						...
						$userDashboards = $this->model->sessionUserDasboards($_SESSION['idUser']);
						$registerInfoSession = $this->model->insertInfoSession($_SESSION['idUser']);
	...
	public function loginUserToken($token){

			$requestUser = $this->model->loginUserToken($token);
			if(empty($requestUser)){
				$arrResponse = array('status' => false, 'msg' => 'El Token es incorrecto o ya expiro' ); 
			}else{
				$arrData = $requestUser;
				if($arrData['STATUS'] == 1){
					$_SESSION['idUser'] = $arrData['ID'];
	...
	public function loginEnterpriseToken($token){

		 $id = $_GET['id']??'';
		$requestUser = $this->model->loginEnterpriseToken($token);
